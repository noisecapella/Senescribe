...
class UserControllerTest < ActionController::TestCase
  fixtures :users
  #CREATE
  def test_create_with_empty_openid
    get :create 
    assert_redirected_to_index "OpenID authentication failed: ' is not an OpenID URL'"
  end

  def test_create_with_url_which_is_not_a_openid_server
  ...
  def test_profile_view_without_id
    get :profile
    
    assert_redirected_to_index("Couldn't find user without an ID")
  end

  def test_profile_view_of_invalid_id
    assert_raise(ActiveRecord::RecordNotFound) do
      get :profile, {:id => 0}
    end
  end
  ...
  #CREATE_NEW_USER
  def test_create_new_user_without_being_authenticated
  ...
  #UPDATE
  def test_update_while_logged_in_as_different_user
    user = User.find_by_description("George")
  ...
  def test_update_while_not_logged_in
    user = User.find_by_description("George")
    params = {:user => {:email => "t", :description => "u"},
      :id => user.id}
    get :update, params
  ...
  def test_update_with_missing_email
    user = User.find_by_description("George")
